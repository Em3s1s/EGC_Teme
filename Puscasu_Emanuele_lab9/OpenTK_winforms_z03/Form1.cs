...
namespace OpenTK_winforms_z02
{
    public partial class Form1 : Form
    {
        ...
        private void DeseneazaSfera(float raza, int numarSegmente, int texturaId)
        {
            GL.Enable(EnableCap.Texture2D);
            GL.BindTexture(TextureTarget.Texture2D, texturaId);

            GL.Begin(PrimitiveType.Triangles);

            for (int i = 0; i < numarSegmente; i++)
            {
                for (int j = 0; j < numarSegmente; j++)
                {
                    float theta1 = (float)(i * 2 * Math.PI / numarSegmente);
                    float theta2 = (float)((i + 1) * 2 * Math.PI / numarSegmente);
                    float phi1 = (float)(j * Math.PI / numarSegmente);
                    float phi2 = (float)((j + 1) * Math.PI / numarSegmente);

                    float x1 = raza * (float)(Math.Sin(phi1) * Math.Cos(theta1));
                    float y1 = raza * (float)Math.Cos(phi1);
                    float z1 = raza * (float)(Math.Sin(phi1) * Math.Sin(theta1));

                    float x2 = raza * (float)(Math.Sin(phi1) * Math.Cos(theta2));
                    float y2 = raza * (float)Math.Cos(phi1);
                    float z2 = raza * (float)(Math.Sin(phi1) * Math.Sin(theta2));

                    float x3 = raza * (float)(Math.Sin(phi2) * Math.Cos(theta1));
                    float y3 = raza * (float)Math.Cos(phi2);
                    float z3 = raza * (float)(Math.Sin(phi2) * Math.Sin(theta1));

                    float x4 = raza * (float)(Math.Sin(phi2) * Math.Cos(theta2));
                    float y4 = raza * (float)Math.Cos(phi2);
                    float z4 = raza * (float)(Math.Sin(phi2) * Math.Sin(theta2));

                    float u1 = (float)i / numarSegmente;
                    float v1 = (float)j / numarSegmente;
                    float u2 = (float)(i + 1) / numarSegmente;
                    float v2 = (float)(j + 1) / numarSegmente;
        ...
        private void DeseneazaCilindru(float raza, float inaltime, int numarSegmente, int texturaId)
        {
            GL.PushMatrix(); // Salvează starea matricei modelview
            GL.Translate(50, 0, 0);
            GL.Enable(EnableCap.Texture2D);
            GL.BindTexture(TextureTarget.Texture2D, texturaId);
            ...
            double segmentAngle = 2.0 * Math.PI / numarSegmente;

            for (int i = 0; i <= numarSegmente; i++)
            {
                double x = raza * Math.Cos(i * segmentAngle);
                double y = raza * Math.Sin(i * segmentAngle);
                double z = 0;

                GL.TexCoord2(0.5 * (1 + Math.Cos(i * segmentAngle)), 0.5 * (1 + Math.Sin(i * segmentAngle)));
                GL.Vertex3(x, y, z);
            }
            ...
            for (int i = 0; i < numarSegmente; i++)
            {
                double x1 = raza * Math.Cos(i * segmentAngle);
                double y1 = raza * Math.Sin(i * segmentAngle);
                double z1 = 0;

                double x2 = raza * Math.Cos((i + 1) * segmentAngle);
                double y2 = raza * Math.Sin((i + 1) * segmentAngle);
                double z2 = 0;

                double x3 = raza * Math.Cos((i + 1) * segmentAngle);
                double y3 = raza * Math.Sin((i + 1) * segmentAngle);
                double z3 = inaltime;

                double x4 = raza * Math.Cos(i * segmentAngle);
                double y4 = raza * Math.Sin(i * segmentAngle);
                double z4 = inaltime;

                GL.TexCoord2(i / (double)numarSegmente, 0);
                GL.Vertex3(x1, y1, z1);

                GL.TexCoord2((i + 1) / (double)numarSegmente, 0);
                GL.Vertex3(x2, y2, z2);

                GL.TexCoord2((i + 1) / (double)numarSegmente, 1);
                GL.Vertex3(x3, y3, z3);

                GL.TexCoord2(i / (double)numarSegmente, 1);
                GL.Vertex3(x4, y4, z4);
            }
        ...
        private void GlControl1_Paint(object sender, PaintEventArgs e)
        {
            ...
            // Asumând că textures[0] este textura dorită (schimbați la nevoie)
            DeseneazaSfera(30, 36, textures[0]);
            float razaCon = 20.0f; // Raza conului
            float inaltimeCon = 40.0f; // Înălțimea conului
            int numarSegmenteCon = 30; // Numărul de segmente pentru a aproxima cercul

            // Asigurați-vă că textures[0] este textura dorită (schimbați la nevoie)
            DeseneazaCilindru(razaCon, inaltimeCon, numarSegmenteCon, textures[0]);
